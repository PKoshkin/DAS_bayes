...
max_time = 1
params = {
    'amin': 75, 'amax': 90, 'bmin': 500, 'bmax': 600,
...
size_long = {'pa': (alen,), 'pb': (blen,), 'pc': (clen,), 'pd': (dlen,), 'pc_a': (clen, a.size),
             'pc_b': (clen, b.size), 'pc_d': (clen, d.size), 'pc_ab': (clen, a.size, b.size),
             'pc_abd': (clen, a.size, b.size, d.size), 'pb_a': (blen, a.size),
             'pb_d': (blen, d.size), 'pb_ad': (blen, a.size, d.size)}
size_short = {'pa': (alen,), 'pb': (blen,), 'pc': (clen,), 'pd': (dlen,), 'pc_a': (clen, 1),
              'pc_b': (clen, 1), 'pc_d': (clen, 1), 'pc_ab': (clen, 1, 1), 'pc_abd': (clen, 1, 1, 1),
              'pb_a': (blen, 1), 'pb_d': (blen, 1), 'pb_ad': (blen, 1, 1)}
...
if var_number == 3:
    ...
    size_long.update({
        'pb_d': (blen, d.shape[0]), 'pb_ad': (blen, a.size, d.shape[0]), 'generate': (N, a.size, b.size)
    })
...
def test_distribution(f, model, arg_long, arg_short, size_long, size_short):
    ...
    if var.shape != (size_long[0],):
        raise Exception(
            'Function {} for model {} returns distribution support of the wrong size: {} instead of {}.'.format(
                f, model, str(var.shape), str((size_long[0],))
            )
        )
    if prob.shape != size_long:
        raise Exception(
            'Function {} for model {} returns distribution probabilities of the wrong size: {} instead of {}.'.format(
